...
"""Generate a synthetic ZNNUE3 net for CI: valid format, deterministic pseudo-random weights.
...
def main():
    ...
    with open(sys.argv[1], "wb") as out:
        ...
        out.write(struct.pack(f"<{HIDDEN}h", *(rng.randint(-50, 50) for _ in range(HIDDEN))))
        # output weights (own + opponent halves)
        out.write(struct.pack(f"<{2 * HIDDEN}h", *(rng.randint(-50, 50) for _ in range(2 * HIDDEN))))
        # output bias
        out.write(struct.pack("<i", rng.randint(-1000, 1000)))
