...
class Position {
  public:
    ...
    int king_sq(Color c) const { return lsb(pieces(c, KING)); }
    ...
    bool attacked_by(int sq, Color c) const { return attackers_to(sq, c, occupied()); }
    bool in_check() const { return attacked_by(king_sq(stm), ~stm); }
