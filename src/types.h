...
enum : int { NO_SQ = 64, SQUARE_NB = 64 };
...
enum : uint8_t { CR_WK = 1, CR_WQ = 2, CR_BK = 4, CR_BQ = 8, CR_ALL = 15 };
...
class Move {
  ...
  public:
    ...
    static Move none() { return Move(uint16_t(0)); }
    uint16_t raw() const { return d; }
    int from() const { return d & 0x3f; }
    int to() const { return (d >> 6) & 0x3f; }
    uint16_t flag() const { return d >> 12; }
    bool is_none() const { return d == 0; }
    bool operator==(Move m) const { return d == m.d; }
    bool operator!=(Move m) const { return d != m.d; }

    bool is_capture() const { return flag() & 4; }
    bool is_ep() const { return flag() == FLAG_EP; }
    bool is_castle() const { return flag() == FLAG_KCASTLE || flag() == FLAG_QCASTLE; }
    bool is_double() const { return flag() == FLAG_DOUBLE; }
    bool is_promo() const { return flag() >= FLAG_PROMO_N; }
    bool is_quiet() const { return !is_capture() && !is_promo(); }
    PieceType promo_pt() const { return PieceType(KNIGHT + (flag() & 3)); }

    std::string to_uci() const {
        if (is_none()) return "0000";
        std::string s = sq_name(from()) + sq_name(to());
        if (is_promo()) s += "  nbrq"[promo_pt() + 1];  // KNIGHT..QUEEN -> n,b,r,q
        return s;
    }
};
...
inline bool is_mate_score(int v) { return v >= VALUE_MATE_IN_MAX || v <= -VALUE_MATE_IN_MAX; }
...
enum Dir : int { NORTH = 8, SOUTH = -8, EAST = 1, WEST = -1, NE = 9, NW = 7, SE = -7, SW = -9 };
template <int D> inline Bitboard shift(Bitboard b) {
    if constexpr (D == NORTH) return b << 8;
    else if constexpr (D == SOUTH) return b >> 8;
    else if constexpr (D == EAST) return (b & ~FILE_H) << 1;
    else if constexpr (D == WEST) return (b & ~FILE_A) >> 1;
    else if constexpr (D == NE) return (b & ~FILE_H) << 9;
    else if constexpr (D == NW) return (b & ~FILE_A) << 7;
    else if constexpr (D == SE) return (b & ~FILE_H) >> 7;
    else if constexpr (D == SW) return (b & ~FILE_A) >> 9;
}
