...
void init_zobrist() {
    ...
    for (int p = 0; p < 12; p++)
        for (int sq = 0; sq < 64; sq++) Zobrist::Piece[p][sq] = rnd();
    for (int i = 0; i < 16; i++) Zobrist::Castle[i] = rnd();
    for (int f = 0; f < 8; f++) Zobrist::EpFile[f] = rnd();
    Zobrist::Side = rnd();
}
...
static bool castleMaskInit = [] {
    for (int i = 0; i < 64; i++) CastleMask[i] = CR_ALL;
...
void Position::make_move(Move m) {
    ...
    if (castling != oldCr) key ^= Zobrist::Castle[oldCr] ^ Zobrist::Castle[castling];
...
void Position::set_fen(const std::string& fen) {
    ...
    if (epStr != "-" && epStr.size() >= 2) {
        ...
        if (pawn_attacks(~stm, ep) & pieces(stm, PAWN)) epSq = ep;
    }
    halfmove = hm;
    fullmove = fm;
    ...
    if (epSq != NO_SQ) key ^= Zobrist::EpFile[file_of(epSq)];
}
