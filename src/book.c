...
// clang-format off
// The canonical Polyglot random array (781 values) from the book-format specification
// (hgm.nubati.net/book_format.html). Order: 768 piece values, 4 castling, 8 en-passant file, 1 turn.
...
Move book_probe(Book *book, const Position *pos)
{
    ...
    return matched[matched_count - 1];
}

// ---- verification gate -------------------------------------------------------------------------------
int run_book_check(void)
{
    // The 9 official test vectors from the Polyglot book-format specification.
    static const struct
    {
        const char *fen;
        uint64_t    key;
