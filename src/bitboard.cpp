...
struct PRNG {
    ...
    uint64_t sparse() { return next() & next() & next(); } // few set bits -> good magic candidates
};
...
Bitboard sliding_attack(int sq, Bitboard occ, const int deltas[4]) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        while (true) {
            ...
            if (std::abs(file_of(ns) - prevFile) > 1) break;
            att |= sq_bb(ns);
            if (occ & sq_bb(ns)) break; // blocker occupies this square; stop after including it
            s = ns;
        }
    }
    return att;
...
void init_magics(bool rook, Bitboard* table, Magic magics[64], const int deltas[4]) {
    ...
    for (int sq = 0; sq < 64; sq++) {
        ...
        for (int i = 0; i < size;) {
            ...
            while (popcount((mask * magics[sq].magic) >> 56) < 6) magics[sq].magic = prng.sparse();
...
void init_bitboards() {
    for (int sq = 0; sq < 64; sq++) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (tf >= 0 && tf < 8 && tr >= 0 && tr < 8) n |= sq_bb(make_square(tf, tr));
            tf = f + kf[i], tr = r + kr[i];
            if (tf >= 0 && tf < 8 && tr >= 0 && tr < 8) k |= sq_bb(make_square(tf, tr));
        }
        KnightAttacks[sq] = n;
        KingAttacks[sq] = k;
    }
