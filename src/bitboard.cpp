...
Bitboard sliding_attack(int square, Bitboard occupancy, const int deltas[4])
{
    ...
    for (int direction = 0; direction < 4; direction++)
    {
        ...
        while (true)
        {
            ...
            if (std::abs(file_of(next_square) - previous_file) > 1)
            {
                break;
            }
...
void init_magics(bool rook, Bitboard *table, Magic magics[64], const int deltas[4])
{
    ...
    for (int square = 0; square < 64; square++)
    {
        ...
        for (int subset_index = 0; subset_index < subset_count;)
        {
            ...
            {
                unsigned table_index = magics[square].index(occupancy[subset_index]);
...
void init_bitboards()
{
    for (int square = 0; square < 64; square++)
    {
        ...
        PawnAttacks[WHITE][square] = shift<NE>(square_bit) | shift<NW>(square_bit);
        PawnAttacks[BLACK][square] = shift<SE>(square_bit) | shift<SW>(square_bit);
